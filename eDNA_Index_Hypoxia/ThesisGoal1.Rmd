...
# Plot eDNA vs hypoxic threshold
```{r}
...
```{r}
dfsplit <- split(eDNAindxEnvData_cleanYr, eDNAindxEnvData_cleanYr$Species)
for (i in 1:length(dfsplit)) { # For each species: 
    ...
    print(title)
    print(ggplot(dfsplit[[i]], aes(x = DO, y = eDNA_index)) +
      geom_point(color = "orange2", alpha = 0.7) +
      geom_smooth(method = 'gam', se = F, color = "cornflowerblue") +
      ggtitle(title) +
      theme_bw())
}
```
...
for (i in 1:length(dfsplit)) { # For each species: 
    ...
    print("GAM quality")
    AIC(ind_DO_GAM) # quality metric
    summary(ind_DO_GAM)$sp.criterion
}
