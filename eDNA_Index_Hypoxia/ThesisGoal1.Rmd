...
length(unique(COI$Species))
print("copepod species detected")
length(unique(allCopepodReads$Species))
...
COI %>% 
  filter(Technical_Replicate == 3) %>% # finding how many need combining
  group_by(SampleId) %>% 
  summarize(n = n())
...
samples <- allCopepodReads %>% 
  group_by(year(Date_UTC), as.Date(Date_UTC)) %>% 
  summarize(n = n()) 

samples %>% 
  group_by(year(`as.Date(Date_UTC)`)) %>% 
  summarize(n = n())

samplesc <- COI %>% 
  group_by(year(Date_UTC), Date_UTC) %>% 
  summarize(n = n()) 

samplesc %>% 
  group_by(year(Date_UTC)) %>% 
  summarize(n = n())
```
...
results <- full_join(eDNAindxEnvDataYr, classification, by = join_by(Species)) %>% 
...
max(results$temperature)

samples2 <- eDNAindxEnvDataYr %>% 
  group_by(year(Date_UTC), Date_UTC) %>% 
  summarize(n = n()) 
...
ggsave(filename = "Props_Below_Threshold_Season_no23.png", 
...
ggsave(filename = "Props_Below_Threshold_Size_no23.png", 
       path = here("eDNA_Index_Hypoxia", "Plots"), 
       width = 2500, height = 2000, units = "px")
```
...
eDNAGraph_23 <- function(df, # Dataframe of species presence/absence + environmental factors
  ...
  for (i in 1:length(dfsplit)) { # For each species: 
    ...
    filename <- paste(paste(paste(spund, sep = "_", "VS"), sep = "_", envCondName), sep = "_", "no2023")
...
eDNAGraph_23(eDNAindxEnvData_cleanYr, # without 2023
          envCond = "DO", 
          envCondName = "Oxygen", 
...
for (i in 1:length(dfsplit)) { # For each species: 
    ...
    print(title)
    data <- dfsplit[[i]] %>% filter(temperature < 11)
...
# Plot + save
    ...
    ggsave(filename = paste(species, sep = "_", "Scatter_noOut.png"), 
           path = here("eDNA_Index_Hypoxia", "Plots", "DO_Temp_Scatter_noOut"), 
           width = 2500, height = 2000, units = "px")
}
```
...
for (i in 1:length(dfsplit)) {
  ...
  coeffname <- paste(species, sep = "_", "BN_Coeffs")
  dfname <- paste(species, sep = "_", "BN_DF")
